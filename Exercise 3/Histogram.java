...
public class Histogram extends Application {
	...
	public void start(Stage ps) {
		...
		while (true) {
			...
			// check if user enter result and the result only contain integer and smaller
			// than 26
			if (result.isPresent() && (!result.get().matches("[0-9]+") || Integer.parseInt(result.get()) > 26)) {
				invalidInput.showAndWait();
			} else {
				// create a MyPieChart object, if there is no result provide by the user, then
				// return zero in the result by default
